...
export default function CameraOCRScreen({ navigation }: any) {
  ...
  if (!permission) return <View style={styles.centered}><ActivityIndicator size="large" color="#0284c7" /></View>;
  if (!permission.granted) {
    return <View style={styles.centered}>
      <Text style={styles.permissionText}>Camera access is needed to scan receipts</Text>
      <TouchableOpacity style={styles.permissionBtn} onPress={requestPermission}><Text style={styles.permissionBtnText}>Grant Permission</Text></TouchableOpacity>
      <TouchableOpacity style={styles.galleryBtn} onPress={pickFromGallery}><Text style={styles.galleryBtnText}>Pick from Gallery</Text></TouchableOpacity>
    </View>;
  }
  ...
  if (imageUri && !processing) {
    return <ScrollView style={styles.container}>
      ...
      <View style={styles.resultCard}>
        ...
        <View style={styles.field}><Text style={styles.fieldLabel}>Merchant</Text><TextInput style={styles.fieldInput} value={data?.merchant || ''} onChangeText={(t) => setData((d) => d ? { ...d, merchant: t } : null)} placeholder="Merchant name" placeholderTextColor="#94a3b8" /></View>
        <View style={styles.field}><Text style={styles.fieldLabel}>Amount (₹)</Text><TextInput style={styles.fieldInput} value={data?.amount?.toString() || ''} onChangeText={(t) => setData((d) => d ? { ...d, amount: parseFloat(t) || 0 } : null)} placeholder="0.00" placeholderTextColor="#94a3b8" keyboardType="decimal-pad" /></View>
        <View style={styles.field}><Text style={styles.fieldLabel}>Date</Text><TextInput style={styles.fieldInput} value={data?.date || ''} onChangeText={(t) => setData((d) => d ? { ...d, date: t } : null)} placeholder="YYYY-MM-DD" placeholderTextColor="#94a3b8" /></View>
        ...
        <View style={styles.actionRow}>
          <TouchableOpacity style={styles.saveBtn} onPress={saveAsTransaction}><Text style={styles.saveBtnText}>Save as Transaction</Text></TouchableOpacity>
          <TouchableOpacity style={styles.rescanBtn} onPress={resetScanner}><Text style={styles.rescanBtnText}>Scan Again</Text></TouchableOpacity>
        </View>
      </View>
    </ScrollView>;
  }

  if (processing) return <View style={styles.centered}><ActivityIndicator size="large" color="#0284c7" /><Text style={styles.processingText}>Processing receipt...</Text></View>;
  ...
  return <View style={styles.container}>
    ...
    <View style={styles.cameraControls}>
      <TouchableOpacity style={styles.galleryPickBtn} onPress={pickFromGallery}><Text style={styles.galleryPickText}>Gallery</Text></TouchableOpacity>
      <TouchableOpacity style={[styles.captureBtn, capturing && { opacity: 0.5 }]} onPress={capturePhoto} disabled={capturing}>
...
const styles = StyleSheet.create({
  ...
  frameGuide: { width: '80%', height: '40%', borderWidth: 2, borderColor: '#fff', borderRadius: 12, justifyContent: 'center', alignItems: 'center', borderStyle: 'dashed', opacity: 0.7 },
  frameText: { color: '#fff', fontSize: 16, fontWeight: '500' },
  cameraControls: { flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center', padding: 24, backgroundColor: '#000' },
