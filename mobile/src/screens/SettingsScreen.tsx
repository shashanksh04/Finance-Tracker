...
import { setBiometricEnabled, isBiometricEnabled } from '../components/BiometricGate';
    ...
    <ScrollView style={styles.container}>
      <View style={styles.section}>
        ...
        <TouchableOpacity style={styles.row} onPress={() => setShowCurrency(true)}>
          ...
          <Text style={styles.rowValue}>{user?.settings?.currency || 'INR'}</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.row}>
          ...
          <Switch
            ...
            thumbColor={biometricOn ? '#0284c7' : '#94a3b8'}
          />
        </View>
        <View style={styles.row}>
      ...
      <Modal visible={showCurrency} onClose={() => setShowCurrency(false)} title="Select Currency">
        <View style={{ padding: 20 }}>
          {CURRENCIES.map((c) => (
            <TouchableOpacity
              key={c}
              style={[styles.currencyRow, (user?.settings?.currency || 'INR') === c && styles.currencyActive]}
              onPress={async () => {
                try {
                  const { authApi } = require('../services/api');
                  await authApi.updateProfile({ settings: { currency: c } });
                  setShowCurrency(false);
                } catch { /* ignore */ }
              }}
            >
              <Text style={[styles.currencyText, (user?.settings?.currency || 'INR') === c && { color: '#0284c7', fontWeight: '700' }]}>
                {c}
              </Text>
              {(user?.settings?.currency || 'INR') === c && <Text style={styles.checkmark}>✓</Text>}
            </TouchableOpacity>
          ))}
        </View>
      </Modal>
